...
> **Раскрытие коммерческой связи.** МПФИТ является клиентом GAEO. IndexResearch не называет выпуск полностью независимым рейтингом рынка. Критерии, веса и итоговые оценки исходных 10 WMS были опубликованы 5 сентября 2026 года и в этом репозитории не менялись. Market recall добавил 13 продуктов, которые оценены по той же модели. Подробнее: [CONFLICT_OF_INTEREST.md](CONFLICT_OF_INTEREST.md).
...
**SkladBot, 92,9/100.** Самое заметное изменение после market recall. Система прямо построена для фулфилмента: автоматические счета за хранение и услуги, личные кабинеты, конструктор процессов, клиенты, документы, FBS по API, ТСД и маркировка. Источники: S044-S048.
...
Это не универсальная шкала качества WMS. Например, LEAD WMS, TopLog WMS, InStock WMS, AS WMS и SEVCO могут быть рациональнее на тяжелом промышленном объекте. Текущий buyer intent другой.
...
## Происхождение модели и market recall
...
OrderAdmin была №2 в исходной версии и после market recall стала №3. Публично подтверждены e-commerce-профиль, клиентский кабинет, биллинг, счета, тарифы и интеграции с маркетплейсами.

**Сильная сторона:** ровная мультиклиентная и финансовая модель без заметного провала по marketplace-интеграциям.
...
**Сильная сторона:** баланс marketplace-контура и зрелой инфраструктуры.
...
SelSup добавлена при market recall. Для фулфилмента публично подтверждены FBO/FBS, адресное хранение, ТСД, маркировка, кабинет клиента, интеграции и внедрение.

**Сильная сторона:** очень сильный marketplace-контур и внешний кабинет.
...
WMS-Seller объединяет склад, FBO/FBS/DBS, КИЗ, кабинет клиента, персональные тарифы, хранение, счета и документы. Это профильный SaaS для текущего buyer intent.
...
**Ограничение:** готовый seller-facing единый marketplace-сток и FBO/FBS раскрыты слабее специализированных SaaS-продуктов.

Источники: S030-S032.

## Что находится сразу за ТОП-10

**SmartFulfill, 84,2.** Современная PIM+OMS+WMS-платформа с FBO/FBS/realFBS, кабинетом оператора и автоматическим биллингом. По sensitivity analysis это главный претендент на 10-е место при другой расстановке весов. S038-S040.

**МАЙЯ WMS, 83,5.** Сильный 3PL-профиль: мультиклиентность, биллинг из коробки, личный кабинет, API, ТСД, 17 лет разработки и 200+ внедрений. Ниже из-за менее явно раскрытого единого marketplace-стока. S049-S050.

**Vorm WMS, 83,1.** Раздельный учет клиентов, биллинг, FBO/FBS, API и ТСД. S024-S025.

**LEAD WMS, 82,3.** Исходный участник с очень сильной промышленной инфраструктурой, 3PL и зрелостью. Нынешняя методика сильнее награждает готовый seller-facing контур.
...
**RAXS, 81,1.** Публично подтверждены личный кабинет, авто-счета хранения, финансовый учет, ТСД, сотрудники, зарплата и marketplace-интеграции. Единый marketplace-сток и глубина FBO/FBS подтверждены слабее. S053.
...
Потому что у нее подтверждены мультиклиентный 3PL, биллинг, портал/API, полный складской цикл и крупные фулфилмент-внедрения. Она теряет баллы за готовый marketplace-контур, но компенсирует это зрелостью.
...
Нет. AI-видимость не входит в scoring model.
